...
class Pipeline:
    ...
    def run(self) -> None:
        ...
        workers = [
            ...
            threading.Thread(target=self._mt_loop, args=(mt, ui), daemon=True, name="mt"),
        ]
        for w in workers:
            w.start()
    ...
    def _prewarm(self, asr: WhisperASR, mt: LlamaMT) -> None:
    ...
    def _asr_loop(self, asr: WhisperASR) -> None:
        while not self._stop.is_set():
            ...
            try:
                ...
                if transcript.text:
                    # Show JA immediately — independent of MT latency.
